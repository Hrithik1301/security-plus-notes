...
**What it is:** IP Addr's are used to identify a particular domain name
...
there is no flexibility in the no. of ip's we can choose in classfull classification, i.e., we cannot customise the no. of ip addr's we want in classfull classification, but in CIDR we can choose the no. of IP Addr's we want but there are a few limitations as follows 

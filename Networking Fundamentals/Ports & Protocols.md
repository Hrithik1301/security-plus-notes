...
| 110  | POP3     | Downloads emails to device and **deletes from server** (unencrypted)               |
| 143  | IMAP     | Syncs emails — **emails stay on server**, accessible from any device (unencrypted) |
...
tcp/445 — Windows file sharing
SOC angle: Exploited by WannaCry and EternalBlue ransomware. 
Unexpected SMB traffic between workstations = lateral movement indicator. 
Always investigate SMB on non-Windows devices.
